Shader "Water Flow/Water Simple" {
	Properties {
		...
		_MainTexMoveSpeedU ("U Move Speed", Range(-20,6)) = 0.5
		_MainTexMoveSpeedV ("V Move Speed", Range(-20,6)) = 0.5
  ...
  [HideInInspector]_Distance8 ("Distance8", float) = 0
	...
	//	half value1 = _Scale * sin(_Time.w * _Speed  + offsetvert * _Frequency );
		...
		half value1 = _Scale * sin(_Time.w * _Speed + offsetvert * _Frequency  + (v.vertex.x * _OffsetX1) + (v.vertex.z * _OffsetZ1)  );
		half value2 = _Scale * sin(_Time.w * _Speed  + offsetvert * _Frequency  + (v.vertex.x * _OffsetX2) + (v.vertex.z * _OffsetZ2));
		half value3 = _Scale * sin(_Time.w * _Speed  + offsetvert * _Frequency  + (v.vertex.x * _OffsetX3) + (v.vertex.z * _OffsetZ3) );
		half value4 = _Scale * sin(_Time.w * _Speed  + offsetvert * _Frequency  + (v.vertex.x * _OffsetX4) + (v.vertex.z * _OffsetZ4));
		half value5 = _Scale * sin(_Time.w * _Speed  + offsetvert * _Frequency   + (v.vertex.x * _OffsetX5) + (v.vertex.z * _OffsetZ5) );
		half value6 = _Scale * sin(_Time.w * _Speed  + offsetvert * _Frequency  + (v.vertex.x * _OffsetX6) + (v.vertex.z * _OffsetZ6)  );
		half value7 = _Scale * sin(_Time.w * _Speed  + offsetvert * _Frequency  + (v.vertex.x * _OffsetX7) + (v.vertex.z * _OffsetZ7) );
		half value8 = _Scale * sin(_Time.w * _Speed + offsetvert * _Frequency  + (v.vertex.x * _OffsetX8) + (v.vertex.z * _OffsetZ8)  );
		
		float3 worldPos = mul(_Object2World, v.vertex).xyz;
		
		
	//	v.vertex.y += value1; //remove for no waves
	//	v.normal.y += value1; //remove for no waves
